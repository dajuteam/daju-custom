/**
 * 金牌經紀人 Widget (Fixed: Prevent Double Load & Ghost Container)
 * 修正重點：
 * 1. [Fix] 加入 window.HAS_DAJU_EXPERT_RUN 旗標，防止重複執行 (解決載入兩次問題)
 * 2. [Fix] 抓到容器瞬間直接 style.display='none' (解決沒資料還顯示的問題)
 * 3. [Keep] CSS 100% 保持原樣
 */
...
(function() {
  ...
  // ⚡ 防重複執行機制 (解決載入兩次的問題)
  // ============================================================
  if (window.HAS_DAJU_EXPERT_RUN) return;
  window.HAS_DAJU_EXPERT_RUN = true;
  ...
  async function initExpertSystem() {
    ...
    // ★★★ 關鍵修正：一抓到容器，不管怎樣先隱藏！ ★★★
    // 這樣可以避免在等待資料時跑出空白框，也確保沒資料時是隱藏的狀態
    ...
    try {
      ...
      // 只有在渲染成功後，容器才會在 renderExpert 裡面被打開
      initObserver();

    } catch (err) {
      console.error('Expert Widget Error:', err);
      // 發生錯誤，因為一開始已經隱藏了，所以這裡不用做任何事，容器會保持隱藏
    }
  }
  ...
  function renderExpert(container, caseName, allExperts) {
    ...
    // ★★★ 如果沒資料：直接結束，容器保持隱藏狀態 (因為一開始設了 display:none) ★★★
    if (matches.length === 0) {
      return;
    }
