/**
 * Project: Gold Agent Card System (V4.9.2 - 3-Key Standard)
 * Core Logic:
 * 1. 3-Key Split Architecture (Data, Probe, Fail) for Max Performance
 * 2. 15-min Local TTL
 * 3. 304 Keep Alive (Updates Probe Key Only)
 * 4. Auto Height + Smooth Entry
 */
...
(function (window, document) {
  ...
  function injectStyles() {
    if (document.getElementById('daju-expert-css-v492')) return;
    const style = document.createElement('style');
    style.id = 'daju-expert-css-v492';
  ...
  async function unifiedDataEngine() {
    ...
    // 3. Error Cooldown (防止 API 掛掉時瘋狂重試)
    ...
    try {
      ...
      // ✅ 304 Not Modified -> 只更新 Probe Key (極速續命)
